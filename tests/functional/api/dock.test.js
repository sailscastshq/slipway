...
test(
  'Dock never generates executable changes from a source-text approximation',
  {
    world: {
      name: 'configured-slipway',
      context: { deploymentTarget: { slug: 'unverified-schema' } }
    }
  },
  async ({ sails, request, expect }) => {
    const original = {
      service: sails.helpers.dock.getDatabaseService,
      models: sails.helpers.dock.getModelsStatic,
      schema: sails.helpers.dock.getSchema
    }
    let schemaReads = 0
    sails.helpers.dock.getDatabaseService = async () => ({
      service: { id: 1, type: 'postgresql' }
    })
    sails.helpers.dock.getModelsStatic = async () => ({
    ...
    sails.helpers.dock.getSchema = async () => {
      schemaReads++
      return { tables: {} }
    }
    try {
      const response = await request
        .as('genesisUser')
        .get('/api/v1/projects/unverified-schema/dock/diff')
      expect(response).toHaveStatus(400)
      expect(response).toHaveJsonPath('code', 'modelsSnapshotUnavailable')
      expect(response).toHaveJsonPath('statements', [])
      expect(schemaReads).toBe(0)
    } finally {
      sails.helpers.dock.getDatabaseService = original.service
      sails.helpers.dock.getModelsStatic = original.models
      sails.helpers.dock.getSchema = original.schema
    }
  }
)
