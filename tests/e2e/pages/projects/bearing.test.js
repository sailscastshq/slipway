...
const screenshotDataUrl =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADElEQVR42mNk+M/wHwAF/gL+Xhc4VQAAAABJRU5ErkJggg=='
      ...
      await sails.models.bearingfeedback.create({
        ...
        images:
          item[0] === 'Review the new feedback loop'
            ? [{ url: screenshotDataUrl, type: 'image/png', size: 70 }]
            : [],
        category: 'feature',
  ...
  async ({ sails, world, page, browserContext, expect }) => {
    ...
    const seededFeedback = [
      ...
      },
      {
        ...
        images: [{ url: screenshotDataUrl, type: 'image/png', size: 70 }],
        status: 'planned',
        voteCount: 14,
        app: app.id,
        space: space.id
    ...
    const categorySelect = composer.getByRole('combobox', { name: 'Category' })
    ...
    await categorySelect.press('ArrowDown')
    await categorySelect.press('Escape')
