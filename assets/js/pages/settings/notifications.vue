...
const allEvents = [
  { id: 'deploySuccess', label: 'Successful deployments', description: 'Notify when a deployment completes successfully', category: 'Deployments', model: 'notifyOnDeploySuccess' },
  { id: 'deployFailure', label: 'Failed deployments', description: 'Notify when a deployment fails', category: 'Deployments', model: 'notifyOnDeployFailure' },
  { id: 'backupSuccess', label: 'Successful backups', description: 'Notify when a backup completes successfully', category: 'Backups', model: 'notifyOnBackupSuccess' },
  { id: 'backupFailure', label: 'Failed backups', description: 'Notify when a backup fails', category: 'Backups', model: 'notifyOnBackupFailure' },
  { id: 'containerRestart', label: 'Container restarts', description: 'Notify when a container unexpectedly restarts', category: 'System', model: 'notifyOnContainerRestart' },
  { id: 'highResourceUsage', label: 'High resource usage', description: 'Notify when CPU or memory usage exceeds thresholds', category: 'System', model: 'notifyOnHighResourceUsage' }
]

// Define all channels for filtering
const allChannels = [
  { id: 'discord', label: 'Discord', description: 'Receive notifications via Discord webhook' },
  { id: 'telegram', label: 'Telegram', description: 'Receive instant notifications via Telegram bot' },
  { id: 'email', label: 'Email (SMTP)', description: 'Receive notifications via email' }
]
      ...
      <div class="mx-auto max-w-4xl">
        ...
        <form @submit.prevent="save" class="space-y-8">
          ...
          <div v-if="Object.keys(groupedEvents).length > 0">
            <h2 class="mb-4 text-sm font-medium text-gray-900 dark:text-white">Notification Events</h2>
            <div class="space-y-4">
              <div v-for="(events, category) in groupedEvents" :key="category" class="rounded-lg border border-gray-200 dark:border-gray-800">
                <div class="border-b border-gray-200 bg-gray-50/50 px-4 py-2 dark:border-gray-800 dark:bg-gray-900/50">
                  <div class="flex items-center gap-2">
                    <svg class="h-4 w-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="categoryIcons[category]"></svg>
                    <span class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ category }}</span>
                  </div>
                </div>
                <div class="divide-y divide-gray-200 dark:divide-gray-800">
                  <label v-for="event in events" :key="event.id" class="flex cursor-pointer items-center justify-between px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-900/30">
                    <div>
            ...
            <div class="space-y-4">
              ...
              <div v-if="filteredChannels.some(c => c.id === 'discord')" class="rounded-lg border border-gray-200 dark:border-gray-800">
                <div class="flex items-center justify-between px-4 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h3 class="text-sm font-medium text-gray-900 dark:text-white">Discord</h3>
                      <p class="text-xs text-gray-500 dark:text-gray-400">Receive notifications via Discord webhook</p>
                    </div>
                  </div>
                  <label class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" v-model="form.discordEnabled" class="peer sr-only" />
                ...
                <div v-if="form.discordEnabled" class="divide-y divide-gray-200 border-t border-gray-200 dark:divide-gray-800 dark:border-gray-800">
                  <div class="px-4 py-3">
                  ...
                  <div class="px-4 py-3">
                    <button
                      ...
                      :disabled="testing === 'discord' || !form.discordWebhookUrl"
                    ...
                    >
                      {{ testing === 'discord' ? 'Sending...' : 'Send test message' }}
                    </button>
                    <p v-if="testResult?.channel === 'discord'" :class="['mt-2 text-sm', testResult.success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400']">
                      {{ testResult.message }}
                    </p>
                  </div>
                </div>
              ...
              <div v-if="filteredChannels.some(c => c.id === 'telegram')" class="rounded-lg border border-gray-200 dark:border-gray-800">
                ...
                <div v-if="form.telegramEnabled" class="divide-y divide-gray-200 border-t border-gray-200 dark:divide-gray-800 dark:border-gray-800">
                  <div class="px-4 py-3">
                    <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Bot Token</label>
                    <div class="flex items-center gap-2">
                      <input
                  ...
                  <div class="px-4 py-3">
                    <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Chat ID</label>
                    <input
                      type="text"
              ...
              <div v-if="filteredChannels.some(c => c.id === 'email')" class="rounded-lg border border-gray-200 dark:border-gray-800">
                ...
                <div v-if="form.smtpEnabled" class="divide-y divide-gray-200 border-t border-gray-200 dark:divide-gray-800 dark:border-gray-800">
                  <!-- Env vars notice -->
                  <div v-if="smtp.fromEnv" class="flex items-start gap-3 bg-blue-50/50 px-4 py-3 dark:bg-blue-950/20">
                    <svg class="mt-0.5 h-4 w-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p class="text-xs text-blue-700 dark:text-blue-300">
                      SMTP settings are being loaded from your global environment variables (MAIL_HOST, MAIL_USER, etc.). Changes here will override those values.
                    </p>
                  </div>
                  ...
                  <div class="grid gap-4 px-4 py-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        :placeholder="smtp.hasPassword ? '••••••••' : 'Enter password'"
                        class="w-full border-b border-dashed border-gray-200 bg-transparent px-1 py-1.5 text-sm text-gray-900 placeholder-gray-400 focus:border-brand focus:outline-none dark:border-gray-700 dark:text-white dark:placeholder-gray-500"
                      />
                  ...
                  <div class="px-4 py-3">
                    <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Notification Recipients</label>
                    <input
                      type="text"
          ...
          <!-- No results -->
          <div v-if="Object.keys(groupedEvents).length === 0 && filteredChannels.length === 0" class="py-8 text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">No results for "{{ search }}"</p>
          </div>

          <!-- Save button -->
          <div class="flex justify-end border-t border-gray-200 pt-6 dark:border-gray-800">
            <button
              type="submit"
