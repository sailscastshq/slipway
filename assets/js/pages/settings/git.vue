    ...
    <div class="flex items-center justify-between border-b border-gray-200 py-4 pl-4 pr-4 dark:border-gray-800 sm:pl-4 sm:pr-8">
      <div class="flex items-center space-x-3">
        ...
        <nav class="flex items-center space-x-2 text-sm">
          <Link href="/settings" class="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">settings</Link>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-6 sm:px-8 sm:py-8">
      <div class="mx-auto max-w-4xl space-y-8">
        ...
        <section v-if="!githubConfigured">
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">GitHub OAuth Setup</h2>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Configure GitHub OAuth to enable push-to-deploy.
            <a href="https://github.com/settings/developers" target="_blank" class="text-brand hover:underline">Create an OAuth App</a> first.
          </p>

          <div class="mt-4">
            <div class="mb-4 rounded-md border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-900/20">
              <p class="text-xs text-blue-800 dark:text-blue-300">
                <strong>Callback URL:</strong>
                <code class="ml-1 rounded bg-blue-100 px-1.5 py-0.5 font-mono dark:bg-blue-800/50">{{ serverUrl }}/auth/github/callback</code>
              </p>
            </div>
        ...
        <section v-else>
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">GitHub</h2>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Connect your GitHub account to enable push-to-deploy.</p>

          <div class="mt-4">
            <div v-if="githubConnected" class="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-900">
              <div class="flex items-center space-x-3">
                <svg class="h-6 w-6 text-gray-900 dark:text-white" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">Connected as {{ githubUser }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Push to deploy is enabled</p>
                </div>
              </div>
        ...
        <section v-if="githubConfigured && githubConnected">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-medium text-gray-900 dark:text-white">Connected Repositories</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Repositories linked for push-to-deploy.</p>
            </div>
          </div>
          ...
          <div class="mt-4">
            <div v-if="connectedRepos.length > 0" class="space-y-2">
              ...
              >
                <div class="flex items-center space-x-3">
                  <svg v-if="repo.isPrivate" class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">{{ repo.fullName }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                      {{ repo.defaultBranch }} → {{ repo.environment?.slug || 'unlinked' }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center space-x-2">
                  <span v-if="repo.webhookActive" class="text-xs text-green-600 dark:text-green-400">Auto-deploy</span>
            ...
            <div v-else class="rounded-lg border border-dashed border-gray-300 px-6 py-8 text-center dark:border-gray-700">
              <p class="text-sm text-gray-500 dark:text-gray-400">No repositories connected yet.</p>
              <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Connect a repository from your project's settings.</p>
            </div>
          </div>
        </section>
        ...
        <section>
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-medium text-gray-900 dark:text-white">Deploy Tokens</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">API tokens for CI/CD deployments.</p>
            </div>
            <button
          ...
          <div class="mt-4">
            <div v-if="deployTokens.length > 0" class="rounded-lg border border-gray-200 dark:border-gray-700">
                  ...
                  <div class="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <span v-if="token.project" class="ml-2">• {{ token.project.name }}</span>
                        <span class="ml-2">• Used {{ token.usageCount }} times</span>
                      </p>
                    </div>
                  </div>
                  <div class="flex items-center space-x-3">
                    <span class="text-xs text-gray-400">{{ timeAgo(token.lastUsedAt) }}</span>
                    <button
            ...
            <div v-else class="rounded-lg border border-dashed border-gray-300 px-6 py-8 text-center dark:border-gray-700">
              ...
              <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">No deploy tokens yet.</p>
              <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Create a token to use in your CI/CD pipeline.</p>
            </div>
          </div>
        </section>
        ...
        <!-- Usage Example -->
        <section>
          <h3 class="text-sm font-medium text-gray-900 dark:text-white">GitHub Actions Example</h3>
          <pre class="mt-3 overflow-x-auto rounded-lg bg-gray-100 p-4 font-mono text-xs text-gray-700 dark:bg-gray-900 dark:text-gray-300"><code># .github/workflows/deploy.yml
...
jobs:
  deploy:
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="showCreateToken" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="closeTokenModal">
          <div class="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-700 dark:bg-gray-900">
            ...
            <div v-if="createdToken" class="mt-4">
              ...
              <div class="mt-3 flex items-center space-x-2">
                <code class="flex-1 overflow-x-auto rounded-md border border-gray-200 bg-gray-50 px-3 py-2 font-mono text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                  {{ createdToken }}
                </code>
                <button
                  @click="copyToken"
            ...
            <form v-else @submit.prevent="createToken" class="mt-4 space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Scope (optional)</label>
                <select
                  v-model="newToken.projectId"
                  class="mt-1 w-full border-b border-dashed border-gray-200 bg-transparent px-1 py-1.5 text-sm text-gray-900 focus:border-brand focus:outline-none dark:border-gray-700 dark:bg-transparent dark:text-white"
                >
