...
const tabs = computed(() => {
  ...
  if (props.telemetry.hasTelemetry) {
    list.push(
      { id: 'requests', label: 'Requests', count: props.telemetry.requests.total },
...
function statusBg(code) {
  if (code >= 500) return 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400'
  if (code >= 400) return 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400'
      ...
      <div class="mx-auto max-w-5xl">
        ...
        <div v-if="activeTab === 'requests'">
          ...
          <div v-else class="py-16 text-center">
            ...
            <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">No requests recorded in the last hour.</p>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'exceptions'">
          ...
          <div class="mb-6 flex items-center justify-between">
            <div class="flex items-baseline space-x-3">
              <span class="text-2xl font-semibold text-gray-900 dark:text-white">{{ telemetry.exceptions.total }}</span>
              <span class="text-sm text-gray-500 dark:text-gray-400">exceptions in the last hour</span>
            </div>
          </div>
          ...
          <div v-if="telemetry.exceptions.groups.length > 0" class="space-y-2">
            ...
            >
              ...
              >
                <!-- Count badge -->
                <span class="inline-flex h-7 min-w-[1.75rem] shrink-0 items-center justify-center rounded-full bg-red-50 px-2 text-xs font-bold text-red-600 dark:bg-red-900/20 dark:text-red-400">
                  {{ group.count }}
                </span>
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center space-x-2">
                    <span class="text-sm font-medium text-red-600 dark:text-red-400">{{ group.exceptionType }}</span>
                    <span v-if="group.handled" class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-gray-800 dark:text-gray-400">caught</span>
                  </div>
                  <div class="mt-0.5 truncate text-sm text-gray-600 dark:text-gray-400">{{ group.message }}</div>
                </div>

                <!-- Last seen -->
                <span class="shrink-0 text-xs text-gray-400 dark:text-gray-500">{{ timeAgo(group.lastSeen) }}</span>

                <svg
                  :class="['h-4 w-4 shrink-0 text-gray-400 transition-transform', expandedException === idx ? 'rotate-180' : '']"
                  fill="none" stroke="currentColor" viewBox="0 0 24 24"
                >
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              ...
              <Transition
                ...
                leave-to-class="max-h-0 opacity-0"
              >
                <div v-if="expandedException === idx" class="overflow-hidden border-t border-gray-100 dark:border-gray-800">
                  <div class="p-4">
                    <div v-if="group.lastUrl" class="mb-3 flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
                      <span class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-400">{{ group.lastMethod }}</span>
                      <span>{{ group.lastUrl }}</span>
                    </div>
                    <pre v-if="group.lastStackTrace" class="max-h-64 overflow-auto rounded-md bg-gray-50 p-3 text-xs text-gray-700 dark:bg-gray-800 dark:text-gray-300"><code>{{ group.lastStackTrace }}</code></pre>
                    <p v-else class="text-sm text-gray-400 dark:text-gray-500">No stack trace available.</p>
                  </div>
                </div>
              </Transition>
            </div>
          ...
          <!-- No exceptions -->
          <div v-else class="py-16 text-center">
            <svg class="mx-auto h-10 w-10 text-emerald-300 dark:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">No exceptions in the last hour. Looking good!</p>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'setup'" class="mx-auto max-w-lg py-8">
          <div class="text-center">
            <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-brand/10">
              <svg class="h-6 w-6 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <h3 class="mt-4 text-base font-semibold text-gray-900 dark:text-white">Enable App Telemetry</h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Track requests, exceptions, and database queries from your deployed Sails app.
            </p>
          </div>
          ...
          <div class="mt-8 space-y-6">
            ...
            <div class="flex space-x-3">
              <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gray-100 text-xs font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-300">1</span>
              <div>
                <div class="text-sm font-medium text-gray-900 dark:text-white">Install the hook in your Sails app</div>
                <div class="mt-2 rounded-md bg-gray-50 p-3 dark:bg-gray-800">
                  <code class="text-xs text-gray-700 dark:text-gray-300">npm install sails-hook-slipway</code>
                </div>
              </div>
            </div>
            ...
            <!-- Step 2 -->
            <div class="flex space-x-3">
              <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gray-100 text-xs font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-300">2</span>
              <div class="min-w-0 flex-1">
                <div class="text-sm font-medium text-gray-900 dark:text-white">Deploy your app</div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Slipway automatically injects <code class="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-800">SLIPWAY_TELEMETRY_URL</code> and <code class="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-800">SLIPWAY_TELEMETRY_TOKEN</code> into your container.
                </p>
              </div>
            </div>
            ...
            <!-- Step 3 -->
            <div class="flex space-x-3">
              <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gray-100 text-xs font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-300">3</span>
              <div>
                <div class="text-sm font-medium text-gray-900 dark:text-white">That's it</div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  The hook auto-instruments HTTP requests, exceptions, and Waterline queries. Data appears here within seconds.
                </p>
              </div>
            </div>
          </div>
          ...
          <!-- Telemetry token -->
          <div v-if="telemetry.telemetryToken" class="mt-8 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-800/50">
            <div class="text-xs font-medium text-gray-500 dark:text-gray-400">Environment telemetry token</div>
            <div class="mt-2 flex items-center space-x-2">
              <code class="flex-1 truncate rounded bg-white px-2 py-1.5 text-xs text-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ telemetry.telemetryToken }}</code>
              <button
                @click="copyToken"
