...
function formatValue(value, attr) {
  ...
  if (attr?.autoCreatedAt || attr?.autoUpdatedAt) {
    try {
      ...
      return { display: d.toLocaleString(undefined, { dateStyle: 'long', timeStyle: 'medium' }), isNull: false }
    } catch { /* fall through */ }
  }
  if (attr?.type === 'boolean') return { display: value, isBoolean: true, isNull: false }
  if (attr?.encrypt) return { display: '(encrypted)', isEncrypted: true, isNull: false }
  if (attr?.type === 'json' || typeof value === 'object') {
    return { display: JSON.stringify(value, null, 2), isJson: true, isNull: false }
  }
    ...
    <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-800 sm:px-6">
      <div class="flex items-center space-x-3">
        ...
        <nav class="flex items-center space-x-2 text-sm sm:hidden">
          ...
          <span class="font-medium text-gray-900 dark:text-white">#{{ recordId }}</span>
        </nav>
          ...
          <div class="lg:col-span-2">
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
              ...
              <dl class="divide-y divide-gray-100 dark:divide-gray-800">
                <!-- Regular attributes -->
                <div v-for="[name, attr] in regularAttrs" :key="name" class="flex px-4 py-3">
                  <dt class="w-40 shrink-0 text-sm font-medium text-gray-500 dark:text-gray-400">
                    {{ name }}
                  </dt>
                  <dd class="min-w-0 flex-1 text-sm text-gray-900 dark:text-white">
                    <template v-if="formatValue(record[name], attr).isNull">
                      <span class="text-gray-300 dark:text-gray-600">null</span>
                    </template>
                    <template v-else-if="formatValue(record[name], attr).isBoolean">
                      <span
                        :class="[
                    ...
                    <template v-else-if="formatValue(record[name], attr).isEncrypted">
                      <span class="inline-flex rounded-full bg-yellow-50 px-2 py-0.5 text-xs font-medium text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400">
                        encrypted
                      </span>
                    </template>
                ...
                <div v-for="assoc in modelAssociations" :key="assoc.alias" class="flex px-4 py-3">
                  <dt class="w-40 shrink-0 text-sm font-medium text-gray-500 dark:text-gray-400">
                    {{ assoc.alias }}
                    <span class="block text-xs font-normal text-gray-400 dark:text-gray-500">→ {{ assoc.model }}</span>
                  </dt>
                  <dd class="min-w-0 flex-1 text-sm">
                    <template v-if="record[assoc.alias] === null || record[assoc.alias] === undefined">
                      <span class="text-gray-300 dark:text-gray-600">null</span>
                    </template>
                    <Link v-else :href="relatedRecordUrl(assoc.model, record[assoc.alias])" class="font-medium text-gray-900 hover:underline dark:text-white">
            ...
            >
              <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-800">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
                  {{ assoc.alias }}
                  <span class="ml-1 text-xs font-normal text-gray-400 dark:text-gray-500">({{ collectionCount(assoc.alias) }})</span>
                </h3>
                <Link
              ...
              <div v-if="collectionRecords(assoc.alias).length === 0" class="px-4 py-6 text-center text-xs text-gray-400 dark:text-gray-500">
                No related records
              </div>
              <ul v-else class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                >
                  <Link
                    :href="relatedRecordUrl(assoc.collection, related.id || related[Object.keys(related)[0]])"
                    class="text-sm font-medium text-gray-900 hover:underline dark:text-white"
                  >
                    #{{ related.id || related[Object.keys(related)[0]] }}
                  </Link>
                  <span v-if="related.name || related.title || related.email" class="ml-2 text-xs text-gray-500 dark:text-gray-400">
