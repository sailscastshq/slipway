  ...
  <div class="flex h-full flex-col" @click="repoDropdownOpen = false">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-6 sm:px-8 sm:py-8">
      <div class="mx-auto max-w-2xl">
        ...
        <!-- Git Integration -->
        <div class="mt-12">
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">Git Integration</h2>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Connect a GitHub repository for automatic deployments.</p>

          <div class="mt-4">
            <!-- Connected repo card -->
            <div v-if="connectedRepo" class="rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p class="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {{ connectedRepo.deployBranch || connectedRepo.defaultBranch }}
                    </p>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                  <button
                    ...
                    :class="[
                      ...
                      autoDeployForm.autoDeploy ? 'bg-brand' : 'bg-gray-200 dark:bg-gray-700'
                    ]"
                  >
                    <span
                      :class="[
                        'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                        autoDeployForm.autoDeploy ? 'translate-x-4' : 'translate-x-0'
                      ]"
                    />
                  </button>
                  <span class="text-sm text-gray-700 dark:text-gray-300">Auto-deploy on push</span>
                </div>
                <button
            ...
            <div v-else-if="githubConnected">
              <div class="relative" @click.stop>
                ...
                <div v-if="selectedRepo" class="space-y-4">
                  <div class="rounded-lg border border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-900">
                    <div class="flex items-center justify-between">
                      <div class="flex items-center space-x-3">
                        ...
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ selectedRepo.fullName }}</p>
                          <p class="text-xs text-gray-500 dark:text-gray-400">{{ selectedRepo.isPrivate ? 'Private' : 'Public' }} repository</p>
                        </div>
                      </div>
                      <button @click="clearSelectedRepo" class="rounded-md p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <!-- Branch select -->
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Deploy branch</label>
                    <div v-if="loadingBranches" class="flex items-center gap-2 py-1.5">
                      <svg class="h-4 w-4 animate-spin text-gray-400" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                      </svg>
                      <span class="text-sm text-gray-500 dark:text-gray-400">Loading branches...</span>
                    </div>
                    <select
                    ...
                    <p v-else class="py-1.5 text-sm text-gray-500 dark:text-gray-400">{{ selectedRepo.defaultBranch }}</p>
                    <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Pushes to this branch will trigger a deployment.</p>
                  </div>
                ...
                <div v-else>
                  <div class="relative">
                    ...
                    <input
                      ...
                      class="w-full border-b border-dashed border-gray-200 bg-transparent py-1.5 pl-7 pr-8 text-sm text-gray-900 placeholder-gray-400 focus:border-brand focus:outline-none dark:border-gray-700 dark:text-white dark:placeholder-gray-500"
                    />
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <span v-if="repo.isConnected" class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">Connected</span>
                      </button>
                    </div>
                  </Transition>
                </div>
            ...
            <div v-else class="rounded-lg border border-dashed border-gray-300 px-6 py-8 text-center dark:border-gray-700">
              <svg class="mx-auto h-8 w-8 text-gray-400 dark:text-gray-500" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
              <p class="mt-3 text-sm font-medium text-gray-900 dark:text-white">GitHub not connected</p>
