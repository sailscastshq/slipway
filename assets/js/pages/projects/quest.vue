...
const _initialRun = typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('run')
...
const _initialJob = typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('job')
...
const expandedJobHistory = computed(() => {
  ...
  return liveHistory.value
    .filter(h => h.jobName === expandedJob.value && (h.event === 'completed' || h.event === 'failed'))
    .slice(0, 20)
})
  ...
  <div class="flex h-full flex-col">
    ...
    <div class="flex items-center justify-between border-b border-gray-200 py-4 pl-4 pr-4 dark:border-gray-800 sm:pl-4 sm:pr-8">
      ...
      <div class="flex items-center space-x-4">
        ...
        <Tooltip :text="connected ? 'Live updates active' : 'Live updates disconnected'">
          <span class="flex items-center space-x-1.5 text-xs" :class="connected ? 'text-emerald-500' : 'text-gray-400 dark:text-gray-500'">
            <span class="relative flex h-2 w-2">
              <span v-if="connected" class="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span>
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="connected ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'"></span>
            </span>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-6 sm:px-8 sm:py-8">
      <div class="mx-auto max-w-4xl">
        ...
        <!-- Error -->
        <div v-else-if="liveError" class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 dark:border-red-900/50 dark:bg-red-950/20">
        ...
        <div v-else-if="liveJobs.length > 0" class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
              <div class="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Total runs (24h)</div>
              <div class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ stats.total }}</div>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
              <div class="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Succeeded</div>
              <div class="mt-1 text-2xl font-semibold text-emerald-600 dark:text-emerald-400">{{ stats.succeeded }}</div>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
              <div class="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Failed</div>
              <div class="mt-1 text-2xl font-semibold" :class="stats.failed > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'">{{ stats.failed }}</div>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
              <div class="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Success rate</div>
              <div class="mt-1 text-2xl font-semibold" :class="successRateColor(stats.successRate)">{{ stats.successRate }}%</div>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-gray-200 bg-white divide-y divide-gray-200 dark:border-gray-800 dark:bg-gray-950 dark:divide-gray-800">
            ...
            >
                  ...
                  >
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
                      ...
                      <template v-if="jobStatsMap[job.name]">
                        <span class="text-xs" :class="jobStatsMap[job.name].lastStatus === 'completed' ? 'text-gray-500 dark:text-gray-400' : 'text-red-600 dark:text-red-400'">
                          {{ timeAgo(jobStatsMap[job.name].lastRunAt) }}
                          <span class="text-gray-300 dark:text-gray-600">&middot;</span>
                          {{ formatDuration(jobStatsMap[job.name].lastDuration) }}
                          <span class="text-gray-300 dark:text-gray-600">&middot;</span>
                          <template v-if="jobStatsMap[job.name].lastStatus === 'completed'">&#10003;</template>
                          <template v-else>&#10007; failed</template>
                        </span>
                      </template>
                      <!-- Next run -->
                      <span v-if="job.nextRunAt" class="text-xs text-gray-400 dark:text-gray-500">
                  ...
                  <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-800">
                    <div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-2 dark:border-gray-800 dark:bg-gray-900">
                      <div class="flex items-center space-x-2">
                        <span v-if="jobOutputs[job.name].running" class="flex items-center space-x-1 text-xs font-medium text-gray-500 dark:text-gray-400">
                          <SlippyLoader size="h-3 w-3" />
                          <span>Running...</span>
                        </span>
                        <span v-else-if="jobOutputs[job.name].output?.success" class="text-xs font-medium text-emerald-600 dark:text-emerald-400">Completed</span>
                        <span v-else class="text-xs font-medium text-red-600 dark:text-red-400">Failed (exit {{ jobOutputs[job.name].output?.exitCode }})</span>
                      </div>
                      <button
                    ...
                    <div
                      :ref="el => { if (el) logContainers[job.name] = el }"
                    ...
                    >
                      ...
                      <template v-else-if="jobOutputs[job.name].output">
                        <pre v-if="jobOutputs[job.name].output.stdout" class="whitespace-pre-wrap break-all text-gray-700 dark:text-gray-300">{{ stripAnsi(jobOutputs[job.name].output.stdout) }}</pre>
                        <pre v-if="!jobOutputs[job.name].output.success && jobOutputs[job.name].output.stderr" class="whitespace-pre-wrap break-all text-red-600 dark:text-red-400" :class="{ 'mt-2': jobOutputs[job.name].output.stdout }">{{ stripAnsi(jobOutputs[job.name].output.stderr) }}</pre>
                        <div v-if="!jobOutputs[job.name].output.stdout && !jobOutputs[job.name].output.stderr" class="text-gray-500 dark:text-gray-400">No output</div>
                      </template>
                    </div>
                  </div>
                </div>
              ...
              >
                <div v-if="expandedJob === job.name" class="overflow-hidden border-t border-gray-100 dark:border-gray-800">
                  <div class="px-4 py-4">
                    <h4 class="mb-3 text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Recent runs</h4>

                    <div v-if="expandedJobHistory.length === 0" class="py-4 text-center text-sm text-gray-400 dark:text-gray-500">
                      No run history yet
                    </div>

                    <div v-else>
                      <div
                        v-for="(run, idx) in expandedJobHistory"
                        :key="idx"
                      >
                        <button
                        ...
                        >
                          <div class="flex items-center space-x-4">
                            <span class="whitespace-nowrap text-xs text-gray-400 dark:text-gray-500">{{ formatDate(run.recordedAt) }} {{ formatTime(run.recordedAt) }}</span>
                            <span class="whitespace-nowrap font-mono text-xs text-gray-600 dark:text-gray-300">{{ formatDuration(run.duration) }}</span>
                            <span v-if="run.trigger === 'manual'" class="text-[10px] text-gray-400 dark:text-gray-500">manual</span>
                          </div>
                          <Tooltip :text="run.event === 'completed' ? 'Completed successfully' : (run.error || 'Failed')">
                            <span class="opacity-0 group-hover:opacity-100 transition-opacity" :class="run.event === 'completed' ? 'text-gray-500 dark:text-gray-400' : 'text-red-600 dark:text-red-400'">
                              <template v-if="run.event === 'completed'">&#10003;</template>
                              <template v-else>&#10007;</template>
                            </span>
                          </Tooltip>
                        </button>
                        <div v-if="expandedRun === String(idx) && (run.stdout || run.stderr)">
                          <div
                            :ref="el => { if (el) runLogContainers[idx] = el }"
                            class="max-h-60 overflow-y-auto bg-gray-100 p-4 font-mono text-xs leading-5 dark:bg-gray-950"
                          >
                            <pre v-if="run.stdout" class="whitespace-pre-wrap break-all text-gray-700 dark:text-gray-300">{{ stripAnsi(run.stdout) }}</pre>
                            <pre v-if="run.stderr" class="whitespace-pre-wrap break-all text-red-600 dark:text-red-400" :class="{ 'mt-2': run.stdout }">{{ stripAnsi(run.stderr) }}</pre>
                          </div>
                        </div>
                        <div v-else-if="expandedRun === String(idx) && !run.stdout && !run.stderr" class="px-4 py-3 text-xs text-gray-400 dark:text-gray-500">
                          No output recorded
                        </div>
                      </div>
                    </div>
