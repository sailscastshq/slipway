...
const statusText = computed(() => {
  ...
  if (interpretation.value.state === 'proposal') {
    return `${props.allowPast ? 'Use' : 'Will schedule for'} ${
      interpretation.value.label
    } in ${zone.value}. Press Enter or leave the picker to use it.`
  }
  return `${props.allowPast ? 'Selected' : 'Scheduled for'} ${
    interpretation.value.label
  } in ${zone.value}.`
})
...
<template>
  ...
  >
    ...
    <p
      ...
      class="aria-invalid:text-red-700 dark:aria-invalid:text-red-400 text-sm text-gray-600 dark:text-gray-400"
          ...
          >
            <div class="flex min-w-0 items-baseline justify-between gap-3">
              ...
              <p
                ...
                class="wrap-anywhere min-w-0 text-end text-xs text-gray-500 dark:text-gray-400"
              >
                {{ zone }}
              </p>
            </div>
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div
                ...
                dir="ltr"
                class="shadow-xs inline-flex shrink-0 items-center rounded-lg border border-gray-200 bg-gray-50 p-0.5 text-sm font-medium tabular-nums dark:border-gray-700 dark:bg-gray-900"
              >
                <select
                ...
                >
                  <select
                    ...
                    aria-label="Period"
                    class="min-w-16 h-11 cursor-pointer appearance-none rounded-md bg-transparent pe-6 ps-2 text-gray-950 hover:bg-gray-200/60 focus-visible:bg-white focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-gray-950 disabled:cursor-not-allowed disabled:opacity-40 dark:text-white dark:hover:bg-gray-800 dark:focus-visible:bg-gray-800 dark:focus-visible:outline-white"
                    :value="fields.period"
                    :disabled="disabled || readonly"
                  ...
                  <svg
                    ...
                    class="inset-e-2 size-3 pointer-events-none absolute top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400"
                  >
