...
function loadValue(value, current = editor.value) {
  ...
  if (!inspection.supported) {
    warning.value = `Keep editing the source to preserve ${inspection.issues
  ...
  >
      ...
      >
        ...
        >
          <button
            ...
            class="min-h-8 cursor-pointer rounded px-2.5 text-xs font-medium text-gray-600 hover:text-gray-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-950 disabled:cursor-not-allowed aria-pressed:bg-white aria-pressed:text-gray-950 aria-pressed:shadow-sm dark:text-gray-400 dark:hover:text-white dark:focus-visible:outline-white dark:aria-pressed:bg-gray-700 dark:aria-pressed:text-white"
    ...
    <textarea
      ...
      :class="
        ready && mode === 'visual'
          ? 'sr-only'
          : 'min-h-56 wrap-anywhere block w-full min-w-0 resize-y rounded-b-[inherit] bg-transparent p-5 font-mono text-sm/7 outline-none'
      "
      @focus="
        (event) =>
    ...
    >
      ...
      >
        ...
        <div class="flex flex-wrap gap-2">
          ...
          ><button
            type="button"
            class="min-h-10 ms-auto cursor-pointer px-2 text-sm"
            @click="closePopup()"
          >
            Cancel
          </button>
