  ...
  fn: async function () {
    ...
    try {
      ...
      if (!uploadsConfig.key || !uploadsConfig.secret || !uploadsConfig.bucket) {
        sails.log.info('[slipway] Skipping pre-update backup — S3 storage not configured')
        return { skipped: true, reason: 'S3 not configured' }
      }

      // 2. Create a consistent SQLite snapshot via better-sqlite3's backup API
      if (!fs.existsSync(dbPath)) {
        sails.log.info('[slipway] Skipping pre-update backup — database file not found')
      ...
      sails.log.info(`[slipway] Pre-update backup uploaded: ${s3Key} (${sizeBytes} bytes)`)
      return { skipped: false, s3Key, sizeBytes }
    } catch (err) {
      sails.log.warn(`[slipway] Pre-update backup failed (update will continue): ${err.message}`)
