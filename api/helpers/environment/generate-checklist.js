...
module.exports = {
  ...
  fn: async function ({ environmentId }) {
    ...
    const hasServiceOfType = (type) =>
      services.some((s) => s.type === type && s.status === 'running')
    ...
    if (!envVars.SESSION_SECRET) {
      checks.push({
        ...
        severity: 'warning',
        suggestion:
          'Without it, sessions use an insecure default. Generate a random secret below.',
    ...
    if (
      ...
      !hasServiceOfType('postgresql') &&
      !hasServiceOfType('mysql') &&
      !hasServiceOfType('mongodb')
    ) {
      checks.push({
    ...
    if (requirements.redis && !hasServiceOfType('redis')) {
      const isForSessions =
        requirements.redisAdapter === '@sailshq/connect-redis'
      checks.push({
        key: 'app-needs-redis',
        label: isForSessions
    ...
    if (
      ...
      checks.push({
        ...
        severity: 'warning',
        suggestion:
          'Create a database service, or verify the external database is reachable.',
        action: { type: 'add-service', label: 'Add database' }
      })
    }
    ...
    if (
      ...
      checks.push({
        ...
        severity: 'warning',
        suggestion:
          'Create a Redis service, or verify the external Redis is reachable.',
        action: {
          type: 'add-service',
          label: 'Add Redis',
          serviceType: 'redis'
    ...
    if (
      (hasServiceOfType('postgresql') || hasServiceOfType('mysql')) &&
      !envVars.DATABASE_URL
    ) {
      checks.push({
        ...
        severity: 'warning',
        suggestion:
          'The connection URL should have been auto-injected. Check env vars or re-create the service.',
    ...
    if (
      ...
      checks.push({
        ...
        severity: 'info',
        suggestion:
          'Most production apps need a database and session store. Consider adding PostgreSQL and Redis.',
