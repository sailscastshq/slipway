module.exports = {
  ...
  fn: async function () {
    ...
    // Telegram settings
    const telegramBotToken = await sails.helpers.setting.get(
      'telegramBotToken',
      ''
    )
    const telegramChatId = await sails.helpers.setting.get('telegramChatId', '')
    const telegramThreadId = await sails.helpers.setting.get(
      'telegramThreadId',
      ''
    )
    ...
    // Slack settings
    const slackWebhookUrl = await sails.helpers.setting.get(
      'slackWebhookUrl',
      ''
    )
    ...
    const smtpHost =
      globalEnvVars.MAIL_HOST ||
      (await sails.helpers.setting.get('smtpHost', ''))
    const smtpPort =
      globalEnvVars.MAIL_PORT ||
      (await sails.helpers.setting.get('smtpPort', '587'))
    const smtpUser =
      globalEnvVars.MAIL_USER ||
      (await sails.helpers.setting.get('smtpUser', ''))
    const smtpFrom =
      globalEnvVars.MAIL_FROM ||
      (await sails.helpers.setting.get('smtpFrom', ''))
    ...
    const notifyOnDeploySuccess = await sails.helpers.setting.get(
      'notifyOnDeploySuccess',
      'true'
    )
    const notifyOnDeployFailure = await sails.helpers.setting.get(
      'notifyOnDeployFailure',
      'true'
    )

    // Notification preferences - backup
    const notifyOnBackupSuccess = await sails.helpers.setting.get(
      'notifyOnBackupSuccess',
      'false'
    )
    const notifyOnBackupFailure = await sails.helpers.setting.get(
    ...
    const notifyOnJobFailure = await sails.helpers.setting.get(
      'notifyOnJobFailure',
      'true'
    )
    ...
    return {
      ...
      props: {
        ...
        smtp: {
          ...
          fromEnv: smtpFromEnv,
          hasPassword: !!(
            globalEnvVars.MAIL_PASSWORD ||
            (await sails.helpers.setting.get('smtpPassword', ''))
          )
        },
        notificationEmails,
        preferences: {
