...
module.exports = {
  ...
  description: 'Get all running containers with latest metrics for the global dashboard.',
  ...
  fn: async function () {
    ...
    if (!user) return { containers: [] }

    // Get all projects for the user's team
    const projects = await Project.find({ team: user.team.id })
    const projectIds = projects.map(p => p.id)
    if (projectIds.length === 0) return { containers: [] }

    // Get all environments for these projects
    const environments = await Environment.find({ project: projectIds })
    const environmentIds = environments.map(e => e.id)
    if (environmentIds.length === 0) return { containers: [] }

    // Get all running apps and services
    const apps = await App.find({ environment: environmentIds, status: 'running' })
    const services = await Service.find({ environment: environmentIds, status: 'running' })
    ...
    if (containerNames.length === 0) return { containers: [] }
    ...
    for (const app of apps) {
      ...
      containers.push({
        ...
        environment: env ? { id: env.id, name: env.name, slug: env.slug } : null,
