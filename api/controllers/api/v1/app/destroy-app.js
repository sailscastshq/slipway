module.exports = {
  ...
  description: 'Stop container and delete an app. Cannot delete the last or default app.',
  ...
  fn: async function ({ projectSlug, environmentSlug, appSlug }) {
    ...
    if (user.teamRole !== 'owner' && user.teamRole !== 'admin') throw 'forbidden'
    ...
    if (appCount <= 1) {
      throw { badRequest: { problems: [{ app: 'Cannot delete the only app in an environment.' }] } }
    }

    // Cannot delete the default app (must reassign first)
    if (app.isDefault) {
      throw { badRequest: { problems: [{ app: 'Cannot delete the default app. Assign another app as default first.' }] } }
    }

    // Stop container if running
    if (app.containerName) {
      try {
        await sails.helpers.docker.stopContainer.with({ containerName: app.containerName })
      } catch (err) {
