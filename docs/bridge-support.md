...
Bridge can open an approved customer page as that customer without replacing the operator's normal application login. It is off by default. Install the support-capable `sails-hook-slipway` shipped with this release, enable Bridge, and redeploy the app after configuring approved pages.
...
A Slipway team owner or administrator opens a record from the declared identity model, chooses **View as this user**, provides a reason, and confirms their current Slipway password. The two-action Klean dialog uses the existing dashed fields. A host Bridge invitation does not grant this capability.
